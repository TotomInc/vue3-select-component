...
describe("v1 assembled Select v0 parity", () => {
  it("shows the placeholder when no option is selected", () => {
    ...
    expect(wrapper.get("[data-v1-select-value]").text()).toBe("Select a country");
  });
  ...
  it("selects the active option with Enter", async () => {
    ...
    expect(wrapper.get("[data-v1-select-value]").text()).toBe("France");
  });
  ...
  it("closes the menu on Escape", async () => {
    ...
    await wrapper.get("[data-v1-select-listbox]").trigger("keydown", { key: "Escape" });
  ...
  it("emits search when typing in searchable mode", async () => {
    ...
    await wrapper.get("[data-v1-select-trigger]").trigger("click");
    await wrapper.get("[data-v1-select-input]").setValue("United");
